...
#include "IECore/StringData.h"
#include "IECore/IntData.h"
#include "IECore/FloatVectorData.h"
#include "IECore/V3fVectorData.h"
#include "IECore/V4fVectorData.h"
...
namespace
{
    ...
    {
        if (numCurvePoints <= 1) return Imath::V3f(1,0,0); // Default for single point or empty curve
    ...
    struct Frame
    {
        ...
        void fromDefault(const Imath::V3f &tangent)
        {
            ...
            if (B.length() < 0.5f) { valid = false; return; } // Should not happen if T is valid and refUp is chosen correctly
            N = safeNormalize(B.cross(T));
            if (N.length() < 0.5f) valid = false;
        }
    };
...
void FeatherAttachBarbs::hashProcessedObject( const ScenePath &path, const Gaffer::Context *context, IECore::MurmurHash &h ) const
{
    ...
    if (inputObject) { // Ensure input object is valid before hashing to avoid errors
        inputObject->hash(h); 
    }
...
IECore::ConstObjectPtr FeatherAttachBarbs::computeProcessedObject( const ScenePath &path, const Gaffer::Context *context, const IECore::Object *inputObject ) const
{
    ...
    for( size_t i = 0; i < shaftsVertsPerCurve.size(); ++i )
    {
        ...
        if (const IntData *idData = runTimeCast<const IntData>(shaftHairId.data.get()))
        {
            ...
            int id = 0;
            if (shaftHairId.interpolation == PrimitiveVariable::Uniform && idData->readable().size() == 1) id = idData->readable()[0];
            else if ((shaftHairId.interpolation == PrimitiveVariable::Uniform || shaftHairId.interpolation == PrimitiveVariable::Primitive) && idData->readable().size() > i) id = idData->readable()[i];
            else { /* Warning or skip */ continue; }
        ...
        else if (const IntVectorData *idVecData = runTimeCast<const IntVectorData>(shaftHairId.data.get())) // Handle IntVectorData for Uniform/Primitive
        {
             shaftsHaveIntHairId = true;
             int id = 0;
             if ((shaftHairId.interpolation == PrimitiveVariable::Uniform || shaftHairId.interpolation == PrimitiveVariable::Primitive) && idVecData->readable().size() > i) id = idVecData->readable()[i];
             else if (shaftHairId.interpolation == PrimitiveVariable::Constant && !idVecData->readable().empty() ) id = idVecData->readable()[0]; // Constant applies to all
             else { /* Warning or skip */ continue; }
             intShaftLookup[id] = info;
        }
    ...
    for( size_t barbIdx = 0; barbIdx < numBarbs; ++barbIdx )
    {
        ...
        if (numBarbCVs <= 0) 
        {
            ...
            if(bind_shaftHairId_str_writable) bind_shaftHairId_str_writable->push_back("");
        ...
        if (const IntData *idData = runTimeCast<const IntData>(barbHairId.data.get()))
        {
            ...
            if (barbHairId.interpolation == PrimitiveVariable::Uniform && idData->readable().size() == 1) barb_hair_id_int = idData->readable()[0];
            else if ((barbHairId.interpolation == PrimitiveVariable::Uniform || barbHairId.interpolation == PrimitiveVariable::Primitive) && idData->readable().size() > barbIdx) barb_hair_id_int = idData->readable()[barbIdx];
            else { /* Warning or skip barb */ IECore::msg(IECore::Msg::Warning, staticTypeName(), "Invalid hairId on barb"); continue; }
        }
        else if (const StringData *idData = runTimeCast<const StringData>(barbHairId.data.get()))
        {
            if (barbHairId.interpolation == PrimitiveVariable::Uniform && idData->readable().size() == 1) barb_hair_id_str = idData->readable()[0];
            else if ((barbHairId.interpolation == PrimitiveVariable::Uniform || barbHairId.interpolation == PrimitiveVariable::Primitive) && idData->readable().size() > barbIdx) barb_hair_id_str = idData->readable()[barbIdx];
            else { /* Warning or skip barb */ IECore::msg(IECore::Msg::Warning, staticTypeName(), "Invalid hairId on barb"); continue; }
        }
        else if (const IntVectorData *idVecData = runTimeCast<const IntVectorData>(barbHairId.data.get()))
        {
            barbHasIntHairId = true;
            if ((barbHairId.interpolation == PrimitiveVariable::Uniform || barbHairId.interpolation == PrimitiveVariable::Primitive) && idVecData->readable().size() > barbIdx) barb_hair_id_int = idVecData->readable()[barbIdx];
            else if (barbHairId.interpolation == PrimitiveVariable::Constant && !idVecData->readable().empty() ) barb_hair_id_int = idVecData->readable()[0];
            else { /* Warning or skip barb */ IECore::msg(IECore::Msg::Warning, staticTypeName(), "Invalid hairId on barb"); continue; }
        }
        else if (const StringVectorData *idVecData = runTimeCast<const StringVectorData>(barbHairId.data.get()))
        {
            if ((barbHairId.interpolation == PrimitiveVariable::Uniform || barbHairId.interpolation == PrimitiveVariable::Primitive) && idVecData->readable().size() > barbIdx) barb_hair_id_str = idVecData->readable()[barbIdx];
            else if (barbHairId.interpolation == PrimitiveVariable::Constant && !idVecData->readable().empty() ) barb_hair_id_str = idVecData->readable()[0];
            else { /* Warning or skip barb */ IECore::msg(IECore::Msg::Warning, staticTypeName(), "Invalid hairId on barb"); continue; }
        }
        else
        {
             IECore::msg(IECore::Msg::Warning, staticTypeName(), "Unsupported hairId type on barb"); continue; 
        }
        
        int shaft_pt_id = -1;
        if (const IntData *idData = runTimeCast<const IntData>(barbShaftPointId.data.get()))
        {
            if (barbShaftPointId.interpolation == PrimitiveVariable::Uniform && idData->readable().size() == 1) shaft_pt_id = idData->readable()[0];
            else if ((barbShaftPointId.interpolation == PrimitiveVariable::Uniform || barbShaftPointId.interpolation == PrimitiveVariable::Primitive) && idData->readable().size() > barbIdx) shaft_pt_id = idData->readable()[barbIdx];
            else { /* Warning or skip barb */ IECore::msg(IECore::Msg::Warning, staticTypeName(), "Invalid shaftPointId on barb"); continue; }
        }
        else if (const IntVectorData *idVecData = runTimeCast<const IntVectorData>(barbShaftPointId.data.get()))
        {
             if ((barbShaftPointId.interpolation == PrimitiveVariable::Uniform || barbShaftPointId.interpolation == PrimitiveVariable::Primitive) && idVecData->readable().size() > barbIdx) shaft_pt_id = idVecData->readable()[barbIdx];
             else if (barbShaftPointId.interpolation == PrimitiveVariable::Constant && !idVecData->readable().empty() ) shaft_pt_id = idVecData->readable()[0];
             else { /* Warning or skip barb */ IECore::msg(IECore::Msg::Warning, staticTypeName(), "Invalid shaftPointId on barb"); continue; }
        }
        else
        {
            IECore::msg(IECore::Msg::Warning, staticTypeName(), "Unsupported shaftPointId type on barb"); continue; 
        }
        ...
        else if (barbHasIntHairId != shaftsHaveIntHairId) // Mismatch
        {
            ...
            // Fill with default and continue to next barb
             if(bind_shaftHairId_int_writable) bind_shaftHairId_int_writable->push_back(barb_hair_id_int); else if(bind_shaftHairId_str_writable) bind_shaftHairId_str_writable->push_back(barb_hair_id_str); // Store original barb id
        ...
        if (!foundShaftInfo)
        {
            ...
            // Fill with default and continue to next barb
            if(bind_shaftHairId_int_writable) bind_shaftHairId_int_writable->push_back(barb_hair_id_int); else if(bind_shaftHairId_str_writable) bind_shaftHairId_str_writable->push_back(barb_hair_id_str);
            bind_shaftPointId_data->writable().push_back(shaft_pt_id);
            bind_shaftRest_P_data->writable().push_back(Imath::V3f(0));
            bind_shaftRest_T_data->writable().push_back(Imath::V3f(1,0,0));
            bind_shaftRest_B_data->writable().push_back(Imath::V3f(0,1,0));
        ...
        if (!frameCalculated)
        {
            ...
            if (currentShaftTangent.length() < 0.5f) { // Invalid tangent
        ...
        {
             if (currentShaftTangent.length() < 0.5f && !frameCalculated) { // Recalculate if not done yet and previous failed
                  currentShaftTangent = calculateTangent(*foundShaftInfo->points, shaftCurveGlobalPointStartIndex, foundShaftInfo->numPoints, shaft_pt_id);
             }
    ...
    else resultCurves->variables["bind_shaftHairId"] = PrimitiveVariable(PrimitiveVariable::Uniform, bind_shaftHairId_data);
