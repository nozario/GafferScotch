...
namespace
{
    ...
    {
        if (numCurvePoints <= 1) return Imath::V3f(1,0,0);
    ...
    struct Frame
    {
        ...
        void fromUpVector(const Imath::V3f &tangent, const Imath::V3f &up)
        {
            ...
            if (B.length() < 0.5f) { valid = false; return; }
        ...
        void fromDefault(const Imath::V3f &tangent)
        {
            ...
            if (std::abs(T.dot(refUp)) > 0.99f) refUp = Imath::V3f(1,0,0);
...
IECore::ConstObjectPtr FeatherDeformBarbs::computeProcessedObject( const ScenePath &path, const Gaffer::Context *context, const IECore::Object *inputObject ) const
{
    ...
    if( barbsVertsPerCurve.empty() ) return inputObject; // No curves
    ...
    if( !shaftUpVectorAttr.empty() )
    {
        ...
        if( !shaftUpVectorData ) IECore::msg( IECore::Msg::Warning, staticTypeName(), boost::format("Animated shaft up-vector attribute '%s' not found or not V3fVectorData (Uniform)." ) % shaftUpVectorAttr );
    }
    ...
    if( !shaftOrientAttr.empty() )
    {
        ...
        if (shaftOrientData) shaftOrientInterpolation = PrimitiveVariable::Vertex;
        else {
            shaftOrientData = animatedShaftsCurves->variableData<V4fVectorData>( shaftOrientAttr, PrimitiveVariable::Uniform );
            if (shaftOrientData) shaftOrientInterpolation = PrimitiveVariable::Uniform;
        }
        if( !shaftOrientData ) IECore::msg( IECore::Msg::Warning, staticTypeName(), boost::format("Animated shaft orientation attribute '%s' not found or not V4fVectorData (Vertex or Uniform)." ) % shaftOrientAttr );
    }
    ...
    for( size_t i = 0; i < animatedShaftsVertsPerCurve.size(); ++i )
    {
        ...
        if (const IntData *idData = runTimeCast<const IntData>(shaftHairId.data.get())) {
            shaftsHaveIntHairId = true; int id = 0;
            ...
            else continue; intShaftLookup[id] = info;
        } else if (const StringData *idData = runTimeCast<const StringData>(shaftHairId.data.get())) {
            shaftsHaveStringHairId = true; std::string id;
            if (shaftHairId.interpolation == PrimitiveVariable::Uniform && idData->readable().size() == 1) id = idData->readable()[0];
            else if ((shaftHairId.interpolation == PrimitiveVariable::Uniform || shaftHairId.interpolation == PrimitiveVariable::Primitive) && idData->readable().size() > i) id = idData->readable()[i];
            else continue; stringShaftLookup[id] = info;
             ...
             else if (shaftHairId.interpolation == PrimitiveVariable::Constant && !idVecData->readable().empty() ) id = idVecData->readable()[0];
             else continue; intShaftLookup[id] = info;
    ...
    const FloatVectorData *barbParamUData = inBarbsCurves->variableData<FloatVectorData>( 
        inBarbsCurves->variables.count("barbParamAttrName") ? 
            inBarbsCurves->variables.at("barbParamAttrName").data->cast<StringData>()->readable() : "curveu", 
    ...
    for( size_t barbIdx = 0; barbIdx < numBarbs; ++barbIdx )
    {
        ...
        if (!animFrameCalculated) {
            ...
            if (currentAnimShaftTangent.length() < 0.5f) animShaftFrame.valid = false;
        ...
        for(int k=0; k < numBarbCVs; ++k) {
            ...
            Imath::M44f M_rot_rest_inv = M_shaft_rest.inverse(); M_rot_rest_inv[3][0]=0; M_rot_rest_inv[3][1]=0; M_rot_rest_inv[3][2]=0; // remove translation
            Imath::M44f M_rot_anim = M_shaft_anim_full; M_rot_anim[3][0]=0; M_rot_anim[3][1]=0; M_rot_anim[3][2]=0; // remove translation
...
{
	return
		Deformer::affectsProcessedObjectBound( input ) ||
		input == inPlug() ||
		input == animatedShaftsPlug() ||
		input == enabledPlug()
	;
